...
2025年2月に横浜フィッシングピアーズのホームページは刷新され、これに伴いデータ取得方式も従来の HTML クローラーから GraphQL API へ移行しました。
この変更により、2025年2月以前の釣果データはサイト上から参照できなくなっています。

過去の釣果データが必要な場合は、本リポジトリに含まれる [旧データベース復元手順](./docs/setup_old_database.md)に従い、データベースを復元してください。

* 本手順で復元されるデータの特性
  - 当時の HTML クローラーによって取得されたアーカイブデータです。
  - 一部期間（システム停止中や2019年の台風19号の影響期間）はデータ欠損があります。
